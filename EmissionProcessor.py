#!/usr/bin/env python3
"""
EmissionProcessor — GUI for laboratory spectroscopy data processing.
...
from .SpeclibViewerTIR import (
    SpeclibViewerTIR,
    _load_hdf,
    _load_sampling_options,
    _INFO_GROUPS,
...
# cmcrameri S-series (perceptually uniform categorical, 100 colors each).
...
_tab20_dark  = [plt.cm.tab20(i) for i in range(0, 20, 2)]   # 10 dark hues
_tab20_light = [plt.cm.tab20(i) for i in range(1, 20, 2)]   # 10 matching light hues

_COLOR_SCHEMES: dict[str, list] = {
    'Standard': _tab20_dark + _tab20_light,
    'Dark':     _dark2 + _lighten_colors(_dark2),
    'batlowS':  [cmcrameri.cm.batlowS(i)  for i in range(100)],
    'glasgowS': [cmcrameri.cm.glasgowS(i) for i in range(100)],
    'hawaiiS':  [cmcrameri.cm.hawaiiS(i)  for i in range(100)],
    'imolaS':   [cmcrameri.cm.imolaS(i)   for i in range(100)],
    'lipariS':  [cmcrameri.cm.lipariS(i)  for i in range(100)],
    'naviaS':   [cmcrameri.cm.naviaS(i)   for i in range(100)],
    'tokyoS':   [cmcrameri.cm.tokyoS(i)   for i in range(100)],
}
...
class AxisLimitsDialog(tk.Toplevel):
    ...
    def __init__(self, viewer: 'EmissionProcessor') -> None:
...
class InstrumentMetricsDialog(tk.Toplevel):
    ...
    def __init__(self, viewer: 'EmissionProcessor',
...
class EmissionProcessor(tk.Tk):

    def __init__(self) -> None:
        super().__init__()
        self.title(f'EmissionProcessor  v{__version__}')
        ...
        # ── Speclib tab state (mirrored from SpeclibViewerTIR) ──────────────
    ...
    # ── Speclib tab ─────────────────────────────────────────────────────────
    # Adapted from SpeclibViewerTIR with excluded/forced endmember additions.
    ...
    def _on_build_library(self) -> None:
        """Launch SpeclibViewerTIR as a modal window; on close, offer to load the export."""
        viewer = SpeclibViewerTIR()
        viewer.transient(self)
        viewer.grab_set()
        viewer.title('SpeclibViewerTIR — Build Library')
        ...
        ans = messagebox.askyesno(
            ...
            'Do you want to load an exported library from SpeclibViewerTIR?',
        )
        if ans:
    ...
    # -----------------------------------------------------------------------
    # Speclib tab methods  (adapted from SpeclibViewerTIR)
    # -----------------------------------------------------------------------
